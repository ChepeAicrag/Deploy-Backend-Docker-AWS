...
6. Listamos con propiedad para visualizar los permisos del archivo. 
       ![img1](./Imagenes/Paso21.png)
7. Procedemos a cambiar los permisos, para ello ejceutamos. Donde name_file.pem es el nombre que le hayas colocado a tus keys. 
    ```bash
    chmod 400 name_file.pem
    ``` 
8. Luego se ejecuta el comando de ejemplo que proporciona la documentación. En mi caso se ejecuta el comando:
    ```bash
    ssh -i "backend-test-key.pem" admin@ec2-3-95-62-144.compute-1.amazonaws.com
    ```
9.  Responde con *yes* para contitnuar.
    
      *Ejecución del paso 8,9 y 10*.
      ![Ejecución de 7 y 8](./Imagenes/Paso22.png) 
10. Y listo, ya estamos conectados a nuestra instancia. 
    ![img1](./Imagenes/Paso23.png)
11. Ahora, les recomiendo ejecutar los demás comandos desde el usuario root, para eso accedemos a el, ejecutando: 
    ```bash
    sudo su
    ```
    ![img1](./Imagenes/Paso24.png)
12. Tammbién les recomiendo hacer un update. En mi caso al ser Debian, se actualiza mendiante:  
    ```bash
    apt-get update
    ```
    ![img1](./Imagenes/Paso25_1.png)
13. Así mismo ejecutar: 
    ```bash
    apt-get upgrade
    ```    
...
### Git 
___
Primeramente verificamos si ya está instalado, esto ejecutando:
```bash
git --version
```
Si no está instalado, como en mi caso, se procede a ejecutar el comando: 
```bash
apt-get install git
```
...
Si no está instalado, procedemos a instalarlo, esto en base a la documentación. En cada distro o imágen se instala diferente, así que es importante que revisen la documentación [aquí](https://docs.docker.com/engine/install). 
...
Finalmente comprobamos ejecutando: 
```bash
docker --version
```
![img1](./Imagenes/Paso30_1.png)
Ahora, para cualquier distro de Linux, se procede con lo siguiente para instalar *Docker Compose* , que es de acuerdo a la documentación que se puede encontrar [aquí](https://docs.docker.com/compose/install).
...
La instalación para Debian es tan sencillo como ejecuta:
...
Y ahora, si vuelvo a probar el endpoint */v1/web/aliments-categories*, en esta ocasión no habrá error, solo retornara un array vacío. 

![img1](./Imagenes/Paso44.png)

Si hago un `POST` desde *insomnia* para agregar información utilizando ya la dirección pública de la instacia *http://3.95.62.144, puedo comprobar que esta funciona correctamente. Así como se aprecia en la imagen: 
